...
class HGTConv(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        self.time_attn_mlp = torch.nn.Linear(self.heads + 1, self.heads)
    ...
    def message(self, k_j: Tensor, q_i: Tensor, v_j: Tensor, rel: Tensor,
            index: Tensor, time_factor, ptr: Optional[Tensor], size_i: Optional[int]) -> Tensor:

        alpha = (q_i * k_j).sum(dim=-1) * rel  # shape: [E, heads]

        if time_factor is not None:
            # Ensure time_factor has shape [E, 1]
            time_factor = time_factor.view(-1, 1)
            # Concatenate along last dim: [E, heads + 1]
            concat = torch.cat([alpha, time_factor], dim=-1)
            # Learnable attention modification
            alpha = self.time_attn_mlp(concat)  # shape: [E, heads]

        alpha = alpha / math.sqrt(q_i.size(-1))
        alpha = softmax(alpha, index, ptr, size_i)  # shape: [E, heads]
        self.latest_attention = alpha.detach().cpu()
