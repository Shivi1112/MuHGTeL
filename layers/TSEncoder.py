...
class AddMetaPathRandomWalkSE(BaseTransform):
    def __init__(self, walk_length: int, device,  attr_name: Optional[str] = 'random_walk_se'):
        ...
        self.device = device
        self.projection = nn.Linear(8, 4).to(self.device)
    ...
    def forward(self, data: HeteroData, metapaths: List) -> HeteroData:
        for metapath in metapaths:
            ...
            if self.attr_name in data[start_node_type]:
                ...
                combined = torch.cat((data[start_node_type][self.attr_name], se), dim=1)
                # Project to fixed dimension
                data[start_node_type][self.attr_name] = self.projection(combined)
            else:
...
#         print('self.k',self.k)
        if num_nodes < self.k + 1:
            # Return zero PE if too small
            return torch.zeros(num_nodes, self.k, device=self.device)
            print('returning zero pe')
        ...
        try:
            ...
            eig_vals, eig_vecs = eigsh(regularized_L, k=k, which='SA', ncv=2*k, tol=1e-3)
            eig_vecs = eig_vecs[:, eig_vals.argsort()]
            eig_vecs = eig_vecs[:, 1:self.k + 1]
        except Exception as e:
            print(f"[WARNING] eigsh failed: {e} → zero PE")
            return torch.zeros(num_nodes, self.k, device=self.device)

        # Pad if needed
        if eig_vecs.shape[1] < self.k:
            pad = np.zeros((eig_vecs.shape[0], self.k - eig_vecs.shape[1]))
            eig_vecs = np.concatenate([eig_vecs, pad], axis=1)

        return torch.from_numpy(eig_vecs).float().to(self.device)
