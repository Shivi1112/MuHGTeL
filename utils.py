...
def parse_datetimes(datetime_strings):
    ...
    for dt_str in datetime_strings:
        dt_str = dt_str.strip()
        try:
            parsed.append(datetime.strptime(dt_str, "%Y-%m-%d %H:%M"))
        except ValueError:
            parsed.append(datetime.strptime(dt_str, "%Y-%m-%d"))
    return parsed
...
class SyntheaDataset(Dataset):
    def __init__(self, root, task_fn):
        self.root = root
    ...
    def _load_and_process(self):
        records = {}
        entities = ['patients', 'encounters', 'conditions', 'procedures', 'medications']
        
        for entity in entities:
            file_path = os.path.join(self.root, f'{entity}.csv')
            if not os.path.exists(file_path):
                raise FileNotFoundError(f"File not found: {file_path}")
                
            df = pd.read_csv(file_path)
            print(f"Loaded {len(df)} {entity} records from {file_path}")
            # Ensure column names are uppercase and handle patient ID column
            df = df.rename(columns=str.upper)
            
            # Special handling for patient ID column
            if 'PATIENT' not in df.columns and 'ID' in df.columns and entity == 'patients':
                df = df.rename(columns={'ID': 'PATIENT'})
                
            records[entity] = df.to_dict('records')
        
        return self.task_fn(records)
        ...
        for sample in self.samples:
            if key in sample:
                ...
                if isinstance(sample[key], list):
                    if sample[key] and isinstance(sample[key][0], list):
                        for sublist in sample[key]:
                            all_tokens.extend(sublist)
                    else:
                        all_tokens.extend(sample[key])
...
def test(data_loader, model, label_tokenizer, return_embeddings=False):
    y_t_all, y_p_all = [], []
    emb_all = []
    ...
    with torch.no_grad():
        ...
        if return_embeddings:
            visit_embeddings = np.concatenate(emb_all, axis=0)
            return y_true, y_prob, visit_embeddings
        else:
